...
async function validerContrat() {
  ...
  /*
   * Sécurité : jsPDF doit être chargé
   */
  if (!window.jspdf || !window.jspdf.jsPDF) {
    al.textContent = '❌ Erreur : le générateur de PDF n\'est pas chargé. Rechargez la page.';
    al.className = 'alert error';
    return;
  }
  ...
  al.className = 'alert';
  al.textContent = '⏳ Génération du contrat et envoi en cours...';
  ...
  try {
    ...
    telechargerPDF(pdf.blob);

    await envoyerEmail(pdf.dataUri);

    al.textContent = '✅ Contrat signé et envoyé avec succès.';
    ...
    al.textContent =
      '❌ Le contrat a été généré, mais l\'envoi de l\'e-mail a échoué. Vérifiez votre connexion et réessayez.';
    al.className = 'alert error';
...
async function envoyerEmail(pdfDataUri) {

  /*
   * Initialisation EmailJS (SDK v4)
   */
  emailjs.init({
  ...
  /*
   * Pièce jointe "Variable Attachment" :
   * EmailJS attend le base64 PUR (sans le préfixe
   * "data:application/pdf;base64,"). On le retire ici.
   */
  ...
  var params = {
    ...
    /*
     * PDF COMPLET EN PIÈCE JOINTE (base64 pur)
     * -> dans le template EmailJS, la pièce jointe
     *    "Variable Attachment" doit avoir pour
     *    "Parameter name" : pdf_contrat
     */
...
function genererPDF() {
  ...
  doc.text('CONDITIONS GENERALES DE RESERVATION', ml, y);
  y += 7;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(107, 122, 153);
  doc.text('SIGNATURE DU CLIENT', ml, y);
  y += 4;

  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(150, 160, 180);
  doc.text('En signant, le client atteste avoir lu et accepte les conditions generales.', ml, y);
  y += 6;
